                ...
                {step === 2 && dnsInstructions && (
                    ...
                    >
                        ...
                        <div className="space-y-3 mb-3 flex-1 overflow-y-auto">
                            ...
                            {/* Nameserver Option (Vercel DNS - fastest propagation) */}
                            {dnsInstructions.isRoot && (
                                <div className="p-4 bg-[var(--color-bg-tertiary)] rounded-xl border border-emerald-500/20">
                                    <div className="flex items-center gap-2 mb-3">
                                        <span className="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded font-bold uppercase">
                                            Opção C — Nameservers Vercel ⚡ Mais Rápido
                                        </span>
                                    </div>
                                    <p className="text-[11px] text-gray-400 mb-3 leading-relaxed">
                                        Delegue o DNS inteiro ao Vercel. O próprio Vercel cria os registros. Propagação em minutos.
                                        <br /><span className="text-gray-500">UOL Host: vá em <strong className="text-gray-300">Domínios → Alterar Servidores DNS</strong> → desative &quot;Utilizar Servidores DNS do UOL&quot;</span>
                                    </p>
                                    <div className="space-y-2">
                                        {['ns1.vercel-dns.com', 'ns2.vercel-dns.com'].map((ns) => (
                                            <div
                                                key={ns}
                                                className="flex items-center justify-between bg-black/20 px-3 py-2 rounded-lg cursor-pointer hover:bg-black/30 transition-colors"
                                                onClick={() => copyToClipboard(ns)}
                                            >
                                                <code className="text-emerald-400 font-mono text-xs">{ns}</code>
                                                <Copy className="w-3 h-3 text-emerald-400 opacity-60 shrink-0" />
                                            </div>
                                        ))}
                                    </div>
                                </div>
